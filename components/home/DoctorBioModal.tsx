...
const TAB_IMAGE_SETS = [
  [
    "/img/doc/docimages/IMG_7209.jpeg",
    "/img/doc/docimages/IMG_9542.jpeg",
    "/img/doc/docimages/photo-output.jpeg",
  ],
...
export const DoctorBioModal = ({ isOpen, onClose }: DoctorBioModalProps) => {
  ...
  const certIcons = [
    ...
    >
      <path
        d="M16.5 18.75h-9m9 0a3 3 0 013 3h-15a3 3 0 013-3m9 0v-4.5A3.375 3.375 0 0012.75 11h-.5A3.375 3.375 0 009 14.25v4.5m3.75-12V3.375c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V6.75"
        strokeLinecap="round"
        strokeLinejoin="round"
        strokeWidth={1.5}
      />
    ...
    switch (activeTabIndex) {
          ...
          <div className="flex flex-col h-full">
            <div>
              <h3 className="text-2xl md:text-3xl font-serif text-white mb-1">
                {t.doctorModal.info.title}
              </h3>
              <h4 className="text-gold font-sans tracking-widest text-xs uppercase mb-4">
                {t.doctorModal.info.subtitle}
              </h4>
                ...
                <li className="flex items-center gap-3">
                  ...
                  <span>
                    <strong className="text-white">
                      {t.doctorModal.info.locationLabel}:
                    </strong>{" "}
                    {t.doctorModal.info.locationValue}
                  </span>
                </li>
                <li className="flex items-center gap-3">
                  <span className="w-1.5 h-1.5 bg-gold rounded-full flex-shrink-0" />
                  <span>
                    <strong className="text-white">
                      {t.doctorModal.info.officeLabel}:
                    </strong>{" "}
                    {t.doctorModal.info.officeValue}
                  </span>
                </li>
                <li className="flex items-center gap-3">
                  <span className="w-1.5 h-1.5 bg-gold rounded-full flex-shrink-0" />
                  <span>
                    <strong className="text-white">
                      {t.doctorModal.info.specialtyLabel}:
                    </strong>{" "}
                    {t.doctorModal.info.specialtyValue}
                  </span>
                </li>
              </ul>
            </div>
            ...
            <div className="mt-auto pt-6 border-t border-white/5 grid grid-cols-3 gap-4">
              <div className="text-center">
                ...
                <p className="text-[10px] text-gray-500 uppercase tracking-widest mt-1 font-sans">
                  {t.doctorModal.info.statsYears}
                </p>
              </div>
              <div className="text-center border-x border-white/5">
                ...
                <p className="text-[10px] text-gray-500 uppercase tracking-widest mt-1 font-sans">
                  {t.doctorModal.info.statsProcedures}
                </p>
              </div>
              <div className="text-center">
                <span className="text-3xl font-serif text-gold font-bold">
                ...
                <p className="text-[10px] text-gray-500 uppercase tracking-widest mt-1 font-sans">
                  {t.doctorModal.info.statsSupport}
                </p>
              </div>
            </div>
          </div>
      ...
      case 1:
        return (
          <div className="flex flex-col h-full">
            <div>
              <h3 className="text-2xl md:text-3xl font-serif text-white mb-1">
                {t.doctorModal.certificates.title}
              </h3>
              <h4 className="text-gold font-sans tracking-widest text-xs uppercase mb-4">
                {t.doctorModal.certificates.subtitle}
              </h4>
            ...
            <div className="flex-1 flex flex-col justify-between">
              {t.doctorModal.certificates.items.map((item, index) => (
                <div
                  key={index}
                  className="flex items-center gap-4 bg-white/5 p-4 rounded-lg border border-white/10 hover:border-gold/30 transition-all duration-300 hover:bg-white/[0.07]"
                >
      ...
      case 2:
        return (
          <div className="flex flex-col h-full">
            <div>
              <h3 className="text-2xl md:text-3xl font-serif text-white mb-1">
                {t.doctorModal.technology.title}
              </h3>
              <h4 className="text-gold font-sans tracking-widest text-xs uppercase mb-4">
                {t.doctorModal.technology.subtitle}
              </h4>
              <p className="mb-4 text-gray-300 font-sans font-light leading-relaxed text-sm">
            ...
            <div className="flex-1 flex flex-col justify-between">
              {(() => {
                ...
                return t.doctorModal.technology.items.map((item, index) => (
                  <div
                    key={index}
                    className="flex items-center gap-4 border-l-2 border-gold pl-5 py-2 hover:border-gold-light transition-colors duration-300 hover:bg-white/[0.02] rounded-r-lg"
                  >
                    {techImages[index] && (
                      <img
                        alt={item.title}
                    ...
                    <div>
                      <strong className="text-white block font-sans text-sm uppercase tracking-wider">
                        {item.title}
                      </strong>
                      <span className="text-sm text-gray-400 font-sans leading-relaxed">
                        {item.description}
                      </span>
                    </div>
                  </div>
      ...
      case 3: {
        const activeTraining =
          safeImageIndex % t.doctorModal.training.items.length;

        return (
          <div className="flex flex-col h-full">
            <div>
              <h3 className="text-2xl md:text-3xl font-serif text-white mb-1">
                {t.doctorModal.training.title}
              </h3>
              <h4 className="text-gold font-sans tracking-widest text-xs uppercase mb-3">
                {t.doctorModal.training.subtitle}
              </h4>
            </div>
            <div className="flex-1 flex flex-col justify-between">
              {t.doctorModal.training.items.map((item, index) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-3">
                    <div
                      className={`w-1.5 h-1.5 rounded-full transition-colors duration-300 flex-shrink-0 ${activeTraining === index ? "bg-gold" : "bg-gray-600"}`}
                    />
                    <div className="flex-1 min-w-0">
                      <h5
                        className={`font-sans font-bold text-xs transition-colors duration-300 ${activeTraining === index ? "text-gold" : "text-white"}`}
                      >
                        {item.title}
                      </h5>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <button
        ...
        className={`absolute inset-0 bg-black/90 backdrop-blur-md transition-opacity duration-500 cursor-default
